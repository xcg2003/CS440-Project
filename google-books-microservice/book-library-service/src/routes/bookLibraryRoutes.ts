...
export async function bookLibraryRoutes(
    ...
    // POST /library/:userId/books  — save a book to the user's library
    app.post<{
    ...
    // GET /library/:userId/books  — retrieve all books for a user
    app.get<{
        Params: { userId: string };
    }>("/library/:userId/books", async (request, reply) => {
        const userId = parseInt(request.params.userId, 10);
    ...
    app.get("/health", async (_request, reply) => {
        return reply.status(200).send({ status: "ok", service: "book-library-service" });
    });
}
